...
async function loadDishRecords() {
  ...
  ingredientNodes.forEach(node => {
    ...
    const lowerKey = ingredientKey.toLowerCase();
    ingredientNameById[id] =
      ingredientNameByKey[lowerKey] ||
  ...
  const records = dishNodes.map(node => {
    ...
    return {
      ...
      imageUrl: PATHS.imageBase + encodeURIComponent(dishKey) + '.png'
    };
  });
...
function bindInputs() {
  ['playerLevel', 'xpNeeded', 'stoveCount', 'holidayMode'].forEach(id => {
    document.getElementById(id).addEventListener('input', renderMyDex);
    document.getElementById(id).addEventListener('change', renderMyDex);
  });
}
...
function renderMyDex() {
  ...
  const availableDishes = allDishRecords.filter(record => record.level <= settings.playerLevel);
  ...
  renderAvailableDishes(regularCandidates, specialDishes, holidayDishes, settings);
}
    ...
    if (!record || settings.xpNeeded <= 0) {
      return `
        <tr class="${rowClass}">
          ...
          <td></td>
          <td></td>
          <td>${settings.stoveCount}</td>
          <td class="note-cell">${settings.xpNeeded <= 0 ? 'Set XP Needed above.' : 'No dish matches this plan.'}</td>
        </tr>
      `;
    }
...
function renderDishCards(containerId, records, fallbackType) {
  ...
  if (records.length === 0) {
    container.innerHTML = '<div class="empty">No dishes from this section are available at your level.</div>';
    return;
  }
  ...
  container.innerHTML = records.map(record => {
    ...
    if (fallbackType === 'Holiday') {
      typeLabel = isHolidayDishActive(record.dishKey, new Date()) ? 'Holiday: Active' : 'Holiday: Inactive';
    }
    ...
    return `
      <article class="dish-card">
        ...
        <div>
          ...
          <p>${escapeHtml(typeLabel)} · Level ${record.level} · ${number(record.xp)} XP · ${escapeHtml(record.durationText)}</p>
...
function renderAvailableDishes(regularCandidates, specialDishes, holidayDishes, settings) {
